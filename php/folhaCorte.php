...
// Para cada modelo cria uma pagina
            ...
            $pdf->MultiCell(0,12,'FOLHA DO CORTE',0,'C');
            $pdf->Ln(1);
   ...
   //         $pdf->Cell(180, 15, '',1, 2,'', false);
            ...
            $pdf->SetFillColor(217,237,247);
            $pdf->SetFont('', 'B');
            $pdf->Cell(26, 6, 'Cor 1',1,0,'C',true);
            $pdf->Cell(26, 6, 'Cor 2',1,0,'C',true);
            $pdf->Cell(26, 6, 'Kilos',1,0,'C',true);
            $pdf->Cell(26, 6, 'Tipo Malha',1,0,'C',true);
            $pdf->Cell(26, 6, 'Fornecedor',1,0,'C',true);
            $pdf->Cell(26, 6, 'Qtd Gasta',1,0,'C',true);
            $pdf->Cell(26, 6, 'Peso p/Cor',1,0,'C',true);
            
            $query3 = sprintf("SELECT * FROM detpedcor WHERE pedido=%s AND modelo=%s",$pid,$row['id']);
            $result3 = mysqli_query($con,$query3);
            if($result3){
                $qtyTotal = 0;
                while ($row3 = mysqli_fetch_array($result3,MYSQLI_ASSOC)) {
                    $cor1 = json_decode($row3['cor1']);
                    $cor2 = json_decode($row3['cor2']);
                    
                    $pdf->Ln();
                    $pdf->SetFillColor(255,255,255);
                    $pdf->Cell(26, 10, utf8_decode($cor1->nome),1,0,'C',true);
                    $pdf->Cell(26, 10, utf8_decode($cor2!=''?$cor2->nome:''),1,0,'C',true);
    ...
    //OBSERVAÇÕES - possivelmente o texto virá da BD
            $pdf->Ln(10);
            $pdf->MultiCell(0, 8, utf8_decode('Observações'));
            $pdf->Ln(1);
            $pdf->MultiCell(180, 6, utf8_decode($row['obscliente']),1, 'L', false);
...
$doc = '../doc/Corte_'.$row0['clienteId'].'_'.$row0['ano'].'_'.$row0['tema'].'.pdf';
$pdf->Output($doc,'F');
