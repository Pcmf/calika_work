...
$query0 = sprintf("SELECT P.*, C.nome AS cnome,C.pasta,C.logo FROM pedido P "
...
// Para cada modelo cria uma pagina
            ...
            $pdf->MultiCell(0,12,utf8_decode('FOLHA DOS BORDADOS'),0,'C');
            $pdf->Ln(1);
            ...
            $pdf->Ln(1);
            $pdf->Cell(180, 30, utf8_decode($row['obscliente']),1, 2,'', false);
           
            //Inserir Tabela 2 Linhas
            $pdf->Ln(10);
            $pdf->SetFillColor(217,237,247);
            $pdf->SetFont('', 'B');
            $pdf->Cell(40, 6, utf8_decode('Inicio Produção'),1,0,'C',true);
            $pdf->Cell(40, 6, utf8_decode('Fim Produção'),1,0,'C',true);
            $pdf->Cell(30, 6, utf8_decode('Peças/Hora'),1,0,'C',true);
            $pdf->Cell(30, 6, 'Custo Bordado',1,0,'C',true);
...
echo 'doc/Bordados_'.$row0['clienteId'].'_'.$row0['ano'].'_'.$row0['tema'].'.pdf';
